...
class Keyword_model extends CI_Model{
    ...
    public function keywordDetails($id) {
        $data = array();
        $query = $this->db->select("*")
					->from($this->table)
...
// echo $this->db->last_query();die;
    ...
    /* Function for get all keyword list start */
    public function allKeywordList() {
        $data = array();
        $query = $this->db->select("*")
                ->from($this->table)
    ...
    /* Function for get all keyword list end */

    /* Function for get keyword list details start */
    public function getKeywordListDetails($slug) {
        $data = array();
        $query = $this->db->select("*")
                    ->from($this->table)
                    ->where(array("$this->table.keyword_slug" => $slug))
                    ->get();
        if ($query->num_rows() > 0) {
            $data = $query->row_array();         
            return $data;  
    ...
    /* Function for get keyword list details end */
