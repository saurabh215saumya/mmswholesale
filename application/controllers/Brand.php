...
// ========================= Admin Section Start ====================
	...
	public function index() {
		...
		$data['allbrands'] = $this->Brand_model->allBrands();
	...
	public function add_newbrand(){
		...
		$this->form_validation->set_rules('brand_slug', 'Brand Slug', "trim|required");
		...
		if($this->form_validation->run() == FALSE ){
			...
			$uploads_dir = dirname($_SERVER["SCRIPT_FILENAME"])."/uploads/brands/";
			...
			if($this->db->insert($this->table, $insert_data)){
				$this->session->set_flashdata('response', '<div class="alert alert-success">Brand Added Successfully</div>');
			} else {
				$this->session->set_flashdata('response', '<div class="alert alert-danger">Failed to Add New Brand.</div>');
			}
	...
	public function edit($id){
		...
		$data['details'] = $this->Brand_model->brandDetails($id);
		$data['controller'] = $this->controller;
	...
	public function update_brand(){
		...
		$data['details'] = $this->Brand_model->brandDetails($brand_id);
		...
		if($this->form_validation->run() == FALSE ){ 
			...
			if (!empty($image['name']) && $image['error'] <= 0) {
				...
				$filename = dirname($_SERVER["SCRIPT_FILENAME"])."/uploads/brands/".$oldImage;
			...
			if($this->db->update($this->table, $data, array('id' => $brand_id))) {
				$this->session->set_flashdata('response', '<div class="alert alert-success">Brand Updated Successfully</div>');
			} else {
				$this->session->set_flashdata('response', '<div class="alert alert-danger">Failed to updated brand.</div>');
			}
			redirect($this->controller, 'refresh');
	...
	public function changestatus() {
		...
		$changes = $this->Brand_model->changeStatus($statusid, $statusVal);
		if($changes) {
			echo $statusRow;
		} else {
			echo 'Server problem';
	...
	public function delete($id) {
		...
		$deleted = $this->Brand_model->deleteRecord($id);
		if($deleted) {
			redirect($this->controller, 'refresh');
		} else {
			redirect($this->controller, 'refresh');
